...
unsafe extern "C" fn callback_get_result_path_info(
    user_data: *mut std::os::raw::c_void,
    deriver: *const raw::StorePath,
) {
    let ret = user_data as *mut Result<ValidStorePathInfo>;

    let deriver = if deriver.is_null() {
        None
    } else {
        let deriver = raw::store_path_clone(deriver);

        let deriver =
            NonNull::new(deriver).expect("nix_store_query_path_info returned a null pointer");
        let deriver = StorePath::new_raw(deriver);
        Some(deriver)
    };

    if (*ret).is_ok() {
        panic!(
            "callback_get_result_path_info: Result must be initialized to Err. Did Nix call us twice?"
        );
    }

    *ret = Ok(ValidStorePathInfo { deriver });
}

fn callback_get_result_path_info_data(
    vec: &mut Result<ValidStorePathInfo>,
) -> *mut std::os::raw::c_void {
    vec as *mut Result<ValidStorePathInfo> as *mut std::os::raw::c_void
}
...
impl Store {
    ...
    pub fn query_path_info(&mut self, path: &StorePath) -> Result<ValidStorePathInfo> {
        let mut r = Err(anyhow::anyhow!(
            "ValidStorePathInfo was not set by Nix C API"
        ));
        unsafe {
            check_call!(raw::store_query_path_info(
                ...
                self.inner.ptr(),
                path.as_ptr(),
                callback_get_result_path_info_data(&mut r),
                Some(callback_get_result_path_info)
            ))
        }?;
        r
    }
