...
use nixops4_resource::{schema::CreateResourceRequest, schema::CreateResourceResponse};
...
struct ExecInProperties {
    ...
    args: Vec<String>,
    stdin: Option<String>,
    // TODO parseJSON  (for convenience and presentation purposes)
}
...
/// schemafy uses `BTreeMap` for `additionalProperties`
type JSONAdditionalProperties = BTreeMap<String, Value>;
...
impl nixops4_resource::framework::ResourceProvider for LocalResourceProvider {
    fn create(&self, request: CreateResourceRequest) -> Result<CreateResourceResponse> {
        eprintln!("LocalResourceProvider::create: {:?}", request);
        ...
        match request.type_.as_str() {
            ...
            "exec" => do_create(request, |p: ExecInProperties| {
                ...
                // FIXME: deadlock! write stdin concurrently with reading
                match p.stdin {
                    Some(stdinstr) => {
                        child
                            .stdin
...
fn do_create<In: for<'de> Deserialize<'de>, Out: serde::Serialize>(
    ...
    let out_properties: JSONAdditionalProperties = out_object.into_iter().collect();
