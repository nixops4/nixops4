...
use crate::schema::{CreateResourceRequest, CreateResourceResponse};
...
/// stdio file descriptors to be safe for logging.
///
/// The stdio file descriptors will point to the following resources:
/// ```text
/// 0: /dev/null
/// 1: stderr
/// 2: stderr
...
pub fn run_main(provider: impl ResourceProvider) {
    eprintln!("provider hi");
    let pipe = {
        let pipe = init_stdio();
        // Turn into file handles
        pipe_fds_to_files(pipe)
    };

    // Read the request from the input
    // let request: CreateResourceRequest = serde_json::from_reader(pipe.in_).unwrap();
